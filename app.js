...
var db = require('./data-sources/db'); // mysql DataSource
var ds = require('./data-sources/ds'); // filesystem DataSource
loopback.AccessToken.attachTo(db);
loopback.Role.attachTo(db);
loopback.ACL.attachTo(db);
loopback.RoleMapping.attachTo(db);
loopback.Scope.attachTo(db);

app.model(loopback.AccessToken);
app.model(loopback.Role);
app.model(loopback.ACL);
app.model(loopback.RoleMapping);
app.model(loopback.Scope);


app.dataSources.db = db;
app.dataSources.ds = ds;
